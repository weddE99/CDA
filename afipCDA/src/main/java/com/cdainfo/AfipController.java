...
public class AfipController {
    ...
    public List<?> getSImpuestosFromClaveFiscal(@PathVariable Long clave_fiscal) {
    	Contribuyente c = contribuyenteRepository.findByNumeroFiscalLike(clave_fiscal);
    ...
    @PostMapping("/afip/altaImpuesto/{clave_fiscal}/{impuesto}")
    public ResponseEntity<?> createImpuestoXContribuyente(@PathVariable Long clave_fiscal, 
    		@PathVariable String impuesto) {
    	Contribuyente c;
    	Impuesto i;
    	c = contribuyenteRepository.findByNumeroFiscalLike(clave_fiscal);
    	i = impuestoRepository.findByTipoImpuestoLike(impuesto);
    	c.getImpuestos().add(i);
    	return new ResponseEntity<>(contribuyenteRepository.save(c), HttpStatus.OK);
    }
}